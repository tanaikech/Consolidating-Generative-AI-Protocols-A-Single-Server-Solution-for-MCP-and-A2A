...
function main(eventObject) {
  let res = null;
  const { A2AObj, MCPObj } = getObjects_();
  if (eventObject.postData) {
    const obj = JSON.parse(eventObject.postData.contents);
    if (obj.method) {
      if (obj.method == "tasks/send" || obj.method == "message/send") {
        res = forA2Aserver(eventObject, A2AObj);
      } else if (["initialize", "notifications/initialized", "notifications/cancelled", "resources/list", "prompts/list", "tools/list", "tools/call"].includes(obj.method)) {
        res = forMCPserver(eventObject, MCPObj);
      }
    }
  } else if (eventObject.pathInfo && eventObject.pathInfo == ".well-known/agent.json") {
    res = forA2Aserver(eventObject, A2AObj);
  }
  return res || ContentService.createTextOutput("{}");
}
...
function getObjects_() {
  const functions = {
    ...
    get_exchange_rate: (object) => {
      ...
      return {
        mcp: { jsonrpc: "2.0", result: { content: [{ type: "text", text: res }], isError: false } },
        a2a: { result: res },
      };
    },
...
function forMCPserver(eventObject, MCPObj) {
  try {
    const object = { eventObject, items: MCPObj };
    const res = new MCPApp({ accessKey: "sample" })
...
function forA2Aserver(eventObject, { agentCard, functions }) {
  const agentCardUrls = [];
  try {
    const object = {
      eventObject,
    ...
    const res = new A2AApp({ accessKey: "sample" })
      .setServices({ lock: LockService.getScriptLock() })
      .server(object);
    console.log(res.getContent()); // Check response.
    return res;
